...
public class OpenAccountController {
    ...
    public ResponseEntity<BaseResponse> openAccount(@RequestBody OpenAccountCommand command){
        ...
        try{
            ...
            return new ResponseEntity<>(new OpenAccountResponse("La cuenta del banc se ha creado exitosamente ".concat("id")), HttpStatus.CREATED);
        }catch (IllegalStateException e){
            logger.log(Level.WARNING, MessageFormat.format("No se pudo generar la cuenta de banco - {0}", e.toString()));
            return new ResponseEntity<>(new BaseResponse(e.toString()), HttpStatus.BAD_REQUEST);
        }catch (Exception e){
            var safeErrorMsg = MessageFormat.format("Errores mientras procesaba el request - {0}", id);
            logger.log(Level.SEVERE, safeErrorMsg, e);
            return new ResponseEntity<>(new OpenAccountResponse(safeErrorMsg, id), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
}
